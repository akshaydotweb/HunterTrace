...
class BacktrackMethod(Enum):
    ...
    DNS_INFRASTRUCTURE = "dns_infrastructure"  # NS/MX/SPF/DKIM/PTR geo
...
class BacktrackResult:
    ...
    # DNS infrastructure signals — dict of sub-signal name → country string
...
class RealIPBacktracker:
    ...
    def backtrack_real_ip(
        ...
        # Technique 8: DNS infrastructure geolocation (VPN-resistant)
        dns_infra_signal = self._analyze_dns_infrastructure(email_headers)
        if dns_infra_signal:
            signals.append(dns_infra_signal)
        ...
        return BacktrackResult(
            ...
            dns_infrastructure_signals=(
                dns_infra_signal.evidence[1:]   # strip summary line, keep detail
                if dns_infra_signal else None
            ),
        )
    ...
    def _analyze_dns_infrastructure(
        ...
        ═══════════════════════════════════════════
        Resolves DNS records published by the sender's domain and maps the
        hosting IPs to countries.  Unlike the IP in the Received: chain, these
        records were registered *before* the email was sent and cannot be
        altered by switching on a VPN.  All five sub-signals are therefore
        VPN-resistant.

        Sub-signals (all fed into a single RealIPSignal):
          1. ns_country      — NS record IPs for sender domain       LR ≈ 8.0
          2. mx_country      — MX record IPs for sender domain       LR ≈ 7.0
          3. spf_netblock    — ip4:/ip6: in SPF TXT record           LR ≈ 9.0
          4. dkim_country    — IP of DKIM selector host              LR ≈ 7.5
          5. ptr_country     — PTR (rDNS) of the first-hop send IP   LR ≈ 6.5

        Confidence tiers:
          ≥3 sub-signals agree → 0.85
          2 sub-signals agree  → 0.70
          1 sub-signal         → 0.45
          0 sub-signals        → None returned

        Network errors are silently caught — the method is best-effort.
        On air-gapped / offline machines all lookups will fail gracefully.
        """
        import socket, re
        from collections import Counter

        evidence: List[str] = []
        country_votes: Counter = Counter()   # country → vote count
        sub_signals: Dict[str, str] = {}     # sub-signal name → country

        # ── Helper: resolve hostname → first public IPv4 ──────────────────
        ...
        # ── Helper: IP → country via GeolocationEnricher or prefix table ──
        def ip_to_country(ip: str) -> Optional[str]:
            if not ip or self._is_private_ip(ip):
                return None
            # Try live geolocation (GeolocationEnricher) if available
            try:
                ...
                if result and getattr(result, "country", None):
                    return result.country
            except Exception:
                pass
            # Fallback: ip-api.com (free, no key required, rate-limited)
            try:
        ...
        # ── Helper: extract sender domain from From / DKIM headers ────────
        def sender_domain() -> Optional[str]:
            # Priority 1: DKIM d= tag (authenticated)
            dkim = email_headers.get("DKIM-Signature", "")
            m = re.search(r'\bd=([^;\s]+)', str(dkim))
            if m:
                return m.group(1).strip().lower()
            # Priority 2: From header
            from_hdr = email_headers.get("From", "")
            m2 = re.search(r'@([\w.\-]+)', str(from_hdr))
            if m2:
        ...
        try:
            ns_ips = []
            # Use dnspython if available, else fallback to getaddrinfo on domain
            try:
                import dns.resolver
                ...
                # dnspython not installed — skip NS lookup gracefully
                pass
            except Exception:
                pass

            for ip in ns_ips[:2]:  # check max 2
                country = ip_to_country(ip)
                if country:
                    evidence.append(f"NS record for {domain} → {ip} → {country}")
                    country_votes[country] += 1
                    sub_signals["ns_country"] = country
                    break
        except Exception:
            pass
        ...
        try:
            try:
                ...
                for rdata in mx_hosts[:2]:
                    mx_host = str(rdata.exchange).rstrip(".")
                    ip = resolve(mx_host)
                    if ip:
                        country = ip_to_country(ip)
                        if country:
                            evidence.append(f"MX record for {domain} → {mx_host} ({ip}) → {country}")
                            country_votes[country] += 1
                            sub_signals["mx_country"] = country
                            break
            except ImportError:
                pass
            except Exception:
        ...
        # ── 3. SPF netblock ──────────────────────────────────────────────
        # Parse TXT record, extract ip4:/ip6: blocks and include: domains,
        # geolocate the first explicit IP found.
        try:
            spf_txt = None
            try:
                import dns.resolver
            ...
            if spf_txt:
                # Extract explicit ip4: / ip6: entries
                for m in re.finditer(r'ip[46]:([0-9a-fA-F.:]+(?:/\d+)?)',
                                     spf_txt):
                    raw = m.group(1).split("/")[0]  # strip CIDR
                    try:
                        test_ip = str(ipaddress.ip_address(raw))
                        if not self._is_private_ip(test_ip):
                            country = ip_to_country(test_ip)
                            if country:
                                evidence.append(
                                    f"SPF ip4: {raw} for {domain} → {country}")
                                country_votes[country] += 2  # weighted +2: explicit netblock
                                sub_signals["spf_netblock"] = country
                                break
                    except ValueError:
                        continue

                # Also resolve include: domains
                if "spf_netblock" not in sub_signals:
                    for m in re.finditer(r'include:([\w.\-]+)', spf_txt):
                        inc_domain = m.group(1)
                        inc_ip = resolve(inc_domain)
                        if inc_ip:
                            country = ip_to_country(inc_ip)
                            if country:
                                evidence.append(
                                    f"SPF include:{inc_domain} → {inc_ip} → {country}")
                                country_votes[country] += 1
                                sub_signals["spf_netblock"] = country
                                break
        except Exception:
        ...
        # ── 4. DKIM selector host ────────────────────────────────────────
        # DKIM-Signature: s=<selector>; d=<domain>
        ...
        try:
            ...
            if sel_m and domain:
                ...
                ip = resolve(dkim_host)
                if ip:
                    country = ip_to_country(ip)
                    if country:
                        evidence.append(
                            f"DKIM selector {dkim_host} → {ip} → {country}")
                        country_votes[country] += 1
                        sub_signals["dkim_country"] = country
        except Exception:
            pass

        # ── 5. PTR (reverse DNS) of sending IP ──────────────────────────
        # The first-hop IP in Received: chain (sender's actual IP or VPN exit).
        # Even for VPN actors, the PTR hostname often encodes the provider
        # and country code (e.g. nl-123.nordvpn.com → NL).
        try:
            # Extract first non-private IP from Received headers
            received_hdrs = email_headers.get("Received", [])
            if isinstance(received_hdrs, str):
                received_hdrs = [received_hdrs]
            origin_ip: Optional[str] = None
            for rcv in received_hdrs:
            ...
            if origin_ip:
                try:
                    ptr_hostname = socket.gethostbyaddr(origin_ip)[0]
                    evidence.append(f"PTR for {origin_ip} → {ptr_hostname}")

                    # Parse country code from PTR hostname
                    # Patterns: .nl., .de., .ru., country-code in label
                    cc_m = re.search(
                        r'\.([a-z]{2})\.(nordvpn|mullvad|expressvpn|'
                        r'surfshark|hidemyass|ipvanish|purevpn|'
                        r'protonvpn|cyberghost|privateinternetaccess)\.',
                        ptr_hostname.lower()
                    )
                    if cc_m:
                        # VPN provider PTR — country code tells us VPN exit country
                        cc = cc_m.group(1).upper()
                        CC_TO_COUNTRY = {
                            "NL": "Netherlands", "DE": "Germany", "US": "United States",
                            "GB": "United Kingdom", "FR": "France", "RU": "Russia",
                            "JP": "Japan", "SG": "Singapore", "HK": "China",
                            "UA": "Ukraine", "PL": "Poland", "SE": "Sweden",
                            "CH": "Switzerland", "CA": "Canada", "AU": "Australia",
                        }
                        ptr_country = CC_TO_COUNTRY.get(cc)
                        if ptr_country:
                            evidence.append(
                                f"PTR country code .{cc.lower()}. → VPN exit: {ptr_country}")
                            # VPN exit PTR is low value (actor chose it) — don't vote
                    else:
                        # Non-VPN PTR — resolve the hostname IP and geolocate
                        ptr_ip = resolve(ptr_hostname)
                        if ptr_ip and ptr_ip != origin_ip:
                            country = ip_to_country(ptr_ip)
                            if country:
                                evidence.append(
                                    f"PTR hostname {ptr_hostname} → {ptr_ip} → {country}")
                                country_votes[country] += 1
                                sub_signals["ptr_country"] = country
                except Exception:
                    pass   # PTR lookup failures are expected and safe to ignore
        except Exception:
            pass
        ...
        best_country, best_votes = country_votes.most_common(1)[0]
        n_agreeing = sum(1 for c, v in sub_signals.items()
                         if v == best_country)

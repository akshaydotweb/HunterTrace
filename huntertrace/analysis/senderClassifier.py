...
Usage (single email):
    from huntertrace.analysis.sender_classifier import classify_sender
    result = classify_sender(header_analysis, email_fingerprints=[fp])

Usage (campaign):
    from huntertrace.analysis.sender_classifier import classify_campaign_sender
...
class HopTimestampAnomalyDetector:
    ...
    def analyze(self, hops: list, raw_headers: Optional[List[str]] = None) -> HopChainAnalysis:
        findings: List[HopAnomalyFinding] = []
        forgery_score = 0.0
        regression_count = 0
        injection_count  = 0
...
def classify_sender(
    ...
    hop_result = _hop_detector.analyze(hops)
...
def classify_campaign_sender(
    ...
    hop_result = _hop_detector.analyze([])   # No hops in fingerprint — campaign mode

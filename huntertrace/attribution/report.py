...
def _load_eml_input(path: Path) -> AnalysisInput:
    ...
    dkim_header = _clean_text(message.get("DKIM-Signature"), max_len=2048)
    if dkim_header:
        dkim_match = _RE_DKIM.search(dkim_header)
        signals.append(
            NormalizedSignal(
                signal_id=f"eml-{idx}",
                name="dkim_domain",
                group="identity",
                value=(dkim_match.group(1) if dkim_match else dkim_header),
                candidate_region=injected_region,
                source="eml.header.dkim-signature",
                trust_label="PARTIALLY_TRUSTED",
                validation_flags=("CLEAN",) if dkim_match else ("SUSPICIOUS",),
                anomaly_detail=None if dkim_match else "Malformed DKIM-Signature d= tag",
            )
        )
        idx += 1

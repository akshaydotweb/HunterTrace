...
class HeaderExtractor:
    ...
    def _parse_header(self, header_text: str, hop_number: int) -> ReceivedHeaderDetail:
        ...
        # ── BUG FIX: timestamp false-positive guard ───────────────────────
        # The loose IPv6 regex  r'\b([a-fA-F0-9]{0,4}:[a-fA-F0-9:]*)\b'
        # matches timestamps like "14:23:01" because time digits are valid
        # hex.  Two guards prevent this permanently:
        #
        #   (a) Strip the '; <date/time>' suffix before running the loose
        #       match — the timestamp always follows the last semicolon in a
        #       Received: header and has no IPv6 content.
        #
        #   (b) Reject any candidate that looks like HH:MM or HH:MM:SS
        #       (1–2 digit groups separated by exactly 2 colons).
        #
        # This fix must never be removed. The root cause is the loose regex
        # matching any hex-colon sequence, and timestamps are valid hex.
        if not ipv6:
            # (a) Remove the timestamp suffix: everything after the last ';'
            _header_no_ts = header_text.rsplit(';', 1)[0]
            _clean = _header_no_ts.replace('[', '').replace(']', '')
            ipv6_loose_match = self.patterns["ipv6_loose"].search(_clean)
            if ipv6_loose_match:
                potential_ipv6 = ipv6_loose_match.group(1)
                # (b) Reject HH:MM and HH:MM:SS patterns — these are clocks, not IPv6
                _is_time = bool(re.match(r'^\d{1,2}:\d{2}(:\d{2})?$', potential_ipv6))
                if self._is_valid_ipv6(potential_ipv6) and ':' in potential_ipv6 and not _is_time:
                    ipv6 = potential_ipv6
...
class IPClassifierLight:
    ...
    def classify_ip(self, ip: str) -> IPClassification:
        ...
        if vpn_result:
            if vpn_result.get("is_mail_relay"):
                # Whitelisted ISP or mail provider — NOT a VPN
                classification = "MAIL_RELAY"
                confidence     = 0.80
                is_vpn         = False
                evidence.append(f"Legitimate mail relay/ISP: {vpn_result.get('_whitelist_reason', '')}")
            elif vpn_result.get("is_vpn"):
            ...
            elif vpn_result.get("is_datacenter"):
                classification = "DATACENTER"
                confidence     = vpn_result.get("confidence", 0.75)
                evidence.append(f"Datacenter: {vpn_result.get('provider', 'Unknown')}")
                provider       = vpn_result.get("provider")
    ...
    def _check_vpn_provider(self, ip: str) -> Optional[Dict]:
        ...
        # ── Step 1: Live ASN/org lookup + whitelist check (single call) ──
        # RATE-LIMIT FIX: Previously Step 0 made a SEPARATE ip-api call just
        # for the whitelist check, then Step 1 made another call for VPN data.
        # Two calls per IP = 2× rate limit consumption → silent failures at
        # 45 req/min, causing webmail extraction to lose real IPs.
        # Now we make ONE call and use the result for both purposes.
        org_name  = ""
        asn_str   = ""
        conn_type = ""
        isp_name  = ""
        try:
            ...
            if resp.status_code == 200:
                ...
                isp_name  = (data.get("isp")  or "").lower()
                conn_type = "proxy" if data.get("proxy") else ("hosting" if data.get("hosting") else "")
        except Exception:
            pass

        # ── Whitelist check (uses the org/isp from the call above) ────────
        # BUG FIX: Yahoo, Azure, CenturyLink, AWS, Windstream etc. are
        # legitimate ISPs and email MTAs — NOT VPN evasion infrastructure.
        # Previously these triggered is_vpn=True, wrongly penalising ACI by −0.15.
        _MAIL_ISP_WHITELIST = {
            "yahoo":                  "Yahoo Mail MTA",
            "oath inc":               "Yahoo/Oath Mail MTA",
            ...
            "lumen technologies":     "Residential ISP (Lumen)",
            ...
            "national informatics":   "Indian Government mail relay",
            "nic.in":                 "Indian Government mail relay",
        }
        _combined = org_name + " " + isp_name
        for _term, _label in _MAIL_ISP_WHITELIST.items():
            if _term in _combined:
                return {
                    "provider":           None,
                    "confidence":         0.0,
                    "is_vpn":             False,
...
class CompletePipeline:
    ...
    def run(self, email_file: str) -> Optional[CompletePipelineResult]:
        ...
        # ── WEBMAIL REAL-IP EXTRACTION (before Stage 2) ───────────────────
        # BUG FIX: run_webmail_extraction() was imported but never called.
        # webmail_extraction field on CompletePipelineResult was always None,
        # causing webmail_leak_rate = 0.0 in every eval run.
        # This must run BEFORE Stage 2 so the real IP enters geolocation.
        webmail_extraction_result = None
        if WEBMAIL_V2_AVAILABLE:
            try:
                with open(email_file, 'r', errors='ignore') as _wf:
                    _raw_email = _wf.read()
                webmail_extraction_result = run_webmail_extraction(_raw_email, verbose=self.verbose)
                if webmail_extraction_result and webmail_extraction_result.real_ip_found:
                    print(f"  [WEBMAIL] Real IP leaked: {webmail_extraction_result.real_ip}"
                          ...
                          f"  ({webmail_extraction_result.provider_name})")
                    # Inject real IP into unique_ips so it gets classified + geolocated
                    if webmail_extraction_result.real_ip not in seen_ipv4:
                        unique_ips.insert(0, webmail_extraction_result.real_ip)
                        seen_ipv4.add(webmail_extraction_result.real_ip)
            except Exception as _we:
                if self.verbose:
                    print(f"  [WARNING] Webmail extraction failed: {_we}")
                webmail_extraction_result = None
        ...
        result = CompletePipelineResult(
            ...
            webmail_extraction=webmail_extraction_result,   # FIX: was always None
        )
